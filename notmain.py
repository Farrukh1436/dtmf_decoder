...
# Lower peak detection threshold and add debugging for peak magnitudes
def find_two_tones(segment, fs):
    ...
    w = windows.hamming(len(segment))  # Changed to Hamming window
    x = segment * w
    ...
    # Debugging: Print FFT values
    print("FFT frequencies:", freqs)
    print("FFT magnitudes:", mag)
    ...
    # Debugging: Print filtered frequencies and magnitudes
    print("Filtered frequencies:", freqs_sub)
    print("Filtered magnitudes:", mag_sub)

    if len(freqs_sub) == 0:
        print("No frequencies in DTMF range.")
        return None, None

    # Peaks
    peaks, props = find_peaks(mag_sub, height=np.max(mag_sub) * 0.1, distance=10)  # Lowered threshold
    print("Detected peaks:", peaks)
    print("Peak properties:", props)

    if len(peaks) == 0:
        print("No peaks detected.")
        return None, None

    # Debugging: Print all peak magnitudes
    peak_magnitudes = [mag_sub[p] for p in peaks]
    ...
    # Debugging: Print top frequencies
    print("Top frequencies:", top_freqs)

    low_cand = [f for f in top_freqs if f < 1100]
    high_cand = [f for f in top_freqs if f >= 1100]

    # Debugging: Print low and high candidates
    print("Low frequency candidates:", low_cand)
    print("High frequency candidates:", high_cand)

    if not low_cand or not high_cand:
        print("No valid low or high frequency candidates.")
        return None, None

    low_freq = low_cand[0]
    high_freq = high_cand[0]

    # Debugging: Print final frequencies
    print("Detected low frequency:", low_freq)
    print("Detected high frequency:", high_freq)
...
def nearest_dtmf_freq(f, dtmf_list, tol=20.0):  # Increased tolerance
    diffs = [abs(f - d) for d in dtmf_list]
    idx = int(np.argmin(diffs))
    print("Frequency differences:", diffs)
    print("Closest frequency index:", idx)
    if diffs[idx] <= tol:
        print("Matched frequency:", dtmf_list[idx])
